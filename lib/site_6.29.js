...
ABCXJS.Tab2Part.prototype.checkTies = function (str) {
    ...
    //desempilha uma ligadura
    let t = this.ties.pop();

    let ties = t.match(/(?:[0-9a-fA-F]'{0,})/g);
    let notas = str.match(/(?:[0-9a-fA-F]'{0,})/g);

    // ties pode ser um subconjunto de str, assim o correto é 
    // procurar cada correspondência de t em str e substituir por ">" 

    let inclui = false;

    // Verificar se pelo menos uma nota em tite coincide com str
    for (let i = 0; !inclui && i < ties.length; i++) {
        for (let j = 0; !inclui && j < notas.length; i++) {
            if (notas[j] === ties[i]) {
                inclui = true;
            }
        }
    ...
    // Substituir cada letra de ties por ">" na mesma posição em notas
    //ao mesmo tempo recriar str e limpar ties encontradas
    str = '';

    for (let i = 0; i < notas.length; i++) {
        var tie = -1;
        for (let j = 0; j < ties.length; i++) {
            if (notas[i] === ties[j]) {
                tie = j;
                break;
            }
...
,//15
    "<div class='enjoy_hint_backgr' style='text-align:left; margin-left:3em'>"+
        ...
        "Você pode entrar em contato comigo pelo e-mail <text class='hint_link'>flavio.luiz.vanni@gmail.com</text><br>"+
...
,//15
     "<div class='enjoy_hint_backgr' style='text-align:left; margin-left:3em'>"+
         ...
         "Puedes contactarme en <text class='hint_link'>flavio.luiz.vanni@gmail.com</text><br>"+
...
,//15
     "<div class='enjoy_hint_backgr' style='text-align:left; margin-left:3em'>"+
         ...
         "You can reach me at <text class='hint_link'>flavio.luiz.vanni@gmail.com</text><br>"+
